...
data.sample %>%  
  ...
  geom_jitter(size = 3, width = 0.1, alpha = 0.5, colour = "red") + 
  # facet_wrap( ~ region, ncol = 5 ) +
  geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
  ylim(c(-6, 6))
```
...
for(i in 1:n_regions) {
  for(j in 1:length(p_h1_levels)) {
    ...
    ggsave(filename = paste0('../plots/raw-data/profits-p_', p_h1_levels[j], '-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
