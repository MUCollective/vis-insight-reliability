...
single_normal <- data_frame(profit = sapply(normal_df, sampled_fn), region = c("A", "B"))
```
...
ggplot(single_normal) + 
  geom_col(aes(x = region, y = profit))
```
...
```{r}
sampled_normal$sample <- rep(1:10, 2)
```
...
ggplot(sampled_normal) +
  geom_col(aes(region, profit))  +
  transition_states(sample, transition_length = 0, state_length = 0.5)
```
