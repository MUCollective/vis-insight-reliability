...
## Anonymisation
We first anonymise the data received from Prolific.
...
```{r}
prolific_fnames <- list.files("../data/final-study/anonymised/", pattern = "final_prolific_(.+).csv")
  
participants <- 
  map_df(
    prolific_fnames,
    ~ read_csv(paste0("../data/final-study/anonymised/", .x), col_types = cols(time_taken = col_character())) %>%
...
# Read data
df_db <- read.csv("../data/final-study/anonymised/final_db_allconds.csv")
...
First we check if there are participants who took the study multiple times:
...
We notice that three participants have completed the study twice. As we didn't anticipate this happening, and hence this was not a preregistered criteria we exclude the second block of trials by these participants, based on the time stamp.
...
Next, we check if the data is complete (by filtering out all participants whose n is not 70):
...
Above, we see that there's one participant whose responses are not complete. Participant with pid = "5c6c403c3dbfc80001abf9f4" completed only one trial (out of 70), and hence was excluded. Most other responses which were excluded were rejected on Prolific.


```{r}
df.adj <- df %>%
  filter((prolific_pid != "807a2eef")) %>%
  group_by(prolific_pid) %>%
  mutate(
