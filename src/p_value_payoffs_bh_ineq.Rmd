...
stimuli_fdr_plot_df %>%
  ggplot(aes(x = fdr, y = factor(nregions), color = method)) + 
  geom_point() + 
  # geom_hline(aes(yintercept = 1)) + 
  # geom_hline(aes(yintercept = 2)) + 
  geom_text(aes(label =(format(round(fdr, 3), nsmall = 3))), nudge_y = 0.2) + 
  theme_minimal() +
  xlim(c(0,.1)) + 
  xlab("E[FDR]")  +
  ylab("nregions") + 
  scale_color_manual(    labels = c("Benjamini\n-Hochberg", "Uncorrected"),
    values = c("#bb3133", "#279e92"))  # + theme(legend.position = "none")


ggsave("fdr_simulation.pdf", device = cairo_pdf, width = 4.5, height = 4.5/2)
```
