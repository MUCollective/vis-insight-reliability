...
We first define a function to simulate unique p-values given a seed, number of comparisons (m), mean (mu) and standard deviation (sd), probability of the null being true (p_null), alpha, and number of trials (ntrials)

```{r}
simulate_pval = function(.seed, mu, sd, m, p_null, alpha, ntrials) {
  set.seed(.seed)
...
df.seed.stimuli = tibble(.seed = 1:1e4) |>
  mutate(.sim = map(.seed, simulate_pval, mu, sd, m, p_null, alpha, ntrials)) |>
  unnest(.sim) |>
...
tibble(.seed = c(6748, 6319, 8533)) |>
  ...
  mutate( p_BH = p.adjust(p_uncorrected, "BH"), p_bonferroni = p.adjust(p_uncorrected, "bonferroni") ) |>
  pivot_longer(cols = starts_with("p_"), names_prefix = "p_", names_to = "method", values_to = "p_val") |>
  group_by(.seed, method, m, trial) |>
  mutate(
    reject_null = p_val < alpha,
...
pilot.exp.stimuli = tibble(.seed = c(6748, 6319, 8533)) |> 
  mutate(.sim = map(.seed, simulate_pval, mu, sd, m, p_null, alpha, ntrials)) |>
  unnest(.sim) |>
  group_by(.seed, m, trial) |>
  filter( (.seed == 6748 & m == 12) | (.seed == 6319 & m == 16) | (.seed == 8533 & m == 20) ) |>
  mutate(p_uncorrected = ifelse(p_uncorrected == 0, 0.001, p_uncorrected)) |> 
  ungroup() |>
  mutate( p_BH = p.adjust(p_uncorrected, "BH"), p_bonferroni = p.adjust(p_uncorrected, "bonferroni") ) |> 
  mutate( data = map2(p_uncorrected, `mu > 0`, generate_data, N, mean, sd) )
```
...
limits = pilot.exp.stimuli |> 
  mutate( 
    ...
    .lower = gamlss.dist::qTF(0.025, mean, se, nu = df),
    .upper = gamlss.dist::qTF(0.975, mean, se, nu = df)
  ) |> 
...
pilot.exp.stimuli |> 
  ...
  filter( m == 12 & trial == 1 & .id == 1) |> 
  mutate( 
    Profit = map_dbl(data, ~ mean(.x)),
    se = map_dbl(data, ~ sd(.x)/sqrt(N)),
    .lower = gamlss.dist::qTF(0.025, Profit, se, nu = df),
    .upper = gamlss.dist::qTF(0.975, Profit, se, nu = df)
  ) |> 
  ggplot() + 
  geom_pointinterval(aes(x = 0, y = Profit, ymin = .lower, ymax = .upper), fill = "#333333", point_size = 5, interval_size = 12) +
...
for (i in m) {
  for (j in 1:ntrials) {
    for (k in 1:i) {
      ...
      ggsave(filename = paste0('../plots/pilot-2023/ci-95/0', k, 'profits-m', i,'-p0.7-', j, '.png'), height = 8, width = 5, units = "in", dpi = 72, plot = p, device = "jpeg")
    }
  }
}
```
