...
n_trials = 10000 # num of trials, here we use a large number for CLT
...
RNG_data = function (n_regions, n, p_null, .seed = 1) {
  sobol(n_trials * n_regions, init = FALSE, dim = n + 1, scrambling = 3, seed = .seed) %>% # gives a multivariate uniform distribution
...
sobol(n_trials * 1, init = FALSE, dim = sample_size + 1, seed = 14) %>% 
  as_tibble(.name_repair = ~ gsub("X.", "V", make.names(., unique = TRUE))) %>%
  rename_at(vars("V1":paste0("V", sample_size)), ~ stringr::str_replace(., 'V', "store_")) %>%
  rename(mu = X)
```

There's some issues with the random number generation, as indicated by the `pairs` plot here:

```{r, fig.width = 16, fig.height = 9}
sobol(200, init = FALSE, dim = 10, seed = 14) %>% 
  as_tibble(.name_repair = ~ gsub("X.", "V", make.names(., unique = TRUE))) %>%
  GGally::ggpairs()
```
...
# saveRDS(data.sample.pnull_0.8, "sim_data_sample-pnull_0.8-n10000.rds")
```
...
```{r}
# data.sample.pnull_0.8 = readRDS("sim_data_sample-pnull_0.8-n10000.rds")
...
We then create a grid of different variables that our optimisation function uses as arguments: alpha level, # of comparisons, and incentive structure. We map over each unique combination of these levels and calculate the optimal alpha level at each point in our grid.
...
df.alpha_optimal = crossing(
  ...
  mutate(optim_alpha = future_pmap(
    list(comparisons, method, TP_mult, FP_mult), 
    function(x, y, z, w) {
...
data.sample.pnull_0.8.summary.alpha = data.sample.pnull_0.8.stats %>%
  ...
  group_split() %>%
  future_map_dfr(
    ~ crossing(., alpha = alphas, nesting(TP_mult = c(1, 1, 1, 9), FP_mult = c(-19, -9, -1, -1)))  %>%
...
data.sample.pnull_0.8.summary.Epayout = data.sample.pnull_0.8.summary.alpha %>%
  mutate(payout = TP*TP_mult + TN - FN + FP*FP_mult) %>%
  summarise(E_payout = mean(payout), .groups = "drop")

saveRDS(data.sample.pnull_0.8.summary.Epayout, file = "sim_data_sample-Epayout-n10000.rds")
```
...
data.sample.pnull_0.8.summary.Epayout = readRDS(file = "sim_data_sample-Epayout-n10000.rds")

data.sample.pnull_0.8.summary.Epayout %>%
  ggplot() +
  geom_line(aes(x = alpha, y = E_payout, group = factor(n_regions), colour = n_regions)) +
  geom_point(
    data = filter(group_by(data.sample.pnull_0.8.summary.Epayout, method, n_regions, TP_mult, FP_mult), E_payout == max(E_payout)), 
    aes(x = alpha, y = E_payout), color = "red", size = 1
  ) +
...
```{r, fig.width = 12, fig.height = 3}
data.sim.alpha_pnull = readRDS(file = "sim-invCDF-alpha_pnull.rds")
