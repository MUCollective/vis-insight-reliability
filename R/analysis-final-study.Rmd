---
title: "Analysis for Odds and Insights: Do Uncertainty Visualisations Improve Quality of Decisions in Visual Analysis"
date: '`r Sys.time()`'
output:  
  # pdf_document:
    # keep_tex: yes
...
```{r predicted-draws-pp-check-1, eval = FALSE, fig.width = 6, fig.height = 3}
test.statistics %>%
  filter(ntrials == 12) %>%
...
draws.fit <- data_grid(df, condition, nesting(ntrials, nregions), adj_trial_id) %>%
  ...
  mutate(
    .category = toupper(as.character(.category)),
    .category = fct_relevel(.category, "TN", "TP", "FN", "FP"),
    .value = .value / ntrials,
    condition = factor(condition, levels = c("raw_data", "ci", "dotplot", "halfeye", "hops_bootstrap", "hops_mean")),
    trial_id = round((adj_trial_id + 1)*35)
  )
...
```{r, eval=FALSE}
# memory eater - peak memory on 12 cores was ~ 34GB, you may want to reduce workers if you have less ram or cores
plan(multisession, workers = 2)
options <- furrr_options(seed = 32)

ame_draws <- df %>%
  select(ntrials, nregions, prolific_pid, adj_trial_id, condition) %>%
  group_by(nregions, condition) %>%
  group_split() %>%
  future_map_dfr(~ 
                   # we do this part (including the marginalization step -- i.e. the summarise())
                   # within the smallest cell of the cross product of the 
                   # predictors that we will marginalize within so that we don't have to
                   # hold the full gigantic table in memory prior to marginalization
  ...
  mutate(
    condition = factor(
      condition,
      levels = c("dotplot", "halfeye", "hops_mean", "hops_bootstrap", "ci", "raw_data")
    ),
...
saveRDS(ame_draws, file = "ame_draws.rds")
```

```{r}
ame_draws <- readRDS("ame_draws.rds")
```
...
```{r scatterplot-for-bias}
ame_draws %>%
  filter(.category != "FDR") %>%
  mutate(.category = factor(.category, levels = c("TP", "TN", "FP", "FN"))) %>%
  group_by(condition, ntrials, .category) %>%
  summarize(retrodictive = mean(.value)) %>%
  left_join(participant_means, by = c("condition", "ntrials", ".category")) %>%
  ggplot(aes(retrodictive, .value)) +
  geom_point(aes(color = .category)) +
  geom_abline(slope = 1, intercept = 0) +
  facet_grid(. ~ ntrials) +
  ylab("Participant mean") +
  theme_classic() +
  labs(title = "Posterior retrodictives vs. participant response means show no consistent bias")  +
  coord_fixed()
```
...
```{r 03-fdr-nregions, fig.width = 8, fig.height = 3.2}
(p1.fdr <- draws.fit %>%
  group_by(ntrials, .draw, .category) %>%
  summarise(.value = mean(.value), .groups = "drop") %>%
  pivot_wider( names_from = .category, values_from = .value ) %>%
  mutate( fp_rate = FP / (FP + TP)) %>%
  ggplot(aes(x = fp_rate, y = factor(ntrials, levels = c("12", "8")))) +
...
# ggsave("figures/figures-paper/03-fdr-nregions-1-chi.pdf", width = 8, height = 3.2)
...
draws.fit %>%
  ...
  mutate( fp_rate = FP / (FP + TP)) %>%
  ggplot(aes(x = fp_rate, y = factor(ntrials))) +
  # geom_halfeyeh(alpha = 0.5,  .width = c(.95, .8, .5)) +
  geom_halfeyeh(alpha = 0.5,  .width = c(.95)) +
  geom_text(data = data.raw.fdr, aes(x = FDR, y = factor(ntrials)), label = "|", size = 8, colour = col_uncorrected) +
...
### Calculation of marginalized estimates
...
```{r, eval = FALSE}
pdf(file = "analysis-final-study_files/figure-latex/prob-rates-3.pdf", useDingbats = FALSE, width = 18, height = 9)
plot.full
dev.off()
```
...
```{r, 08-probs-vis-learning-1, fig.width = 8, fig.height = 5}
df.design <- crossing(condition = c("raw_data", "ci", "dotplot", "halfeye", "hops_bootstrap", "hops_mean"), ntrials = c(8, 12)) %>%
  mutate(ntrials = as.integer(ntrials))

data.raw.fdr2 <- right_join(df.strategy_by_trial, df.design, by = c("ntrials")) %>% filter(method == "uncorrected" & .category != "FDR")
data.bh.fdr2 <- right_join(df.strategy_by_trial, df.design, by = c("ntrials")) %>% filter(method == "bh" & .category != "FDR")
...
(
  plot.full <- draws.fit %>%
    ...
    ggplot() +
    stat_lineribbon(aes(x = trial_id, y = .value)) +
    geom_hline(data = data.raw.fdr2, aes(yintercept = .value), color = col_uncorrected) +
    geom_hline(data = data.bh.fdr2, aes(yintercept = .value), color = col_bh) +
    scale_fill_brewer() + 
    ylim(c(0, 0.65)) +
