...
```{r}
prolific_fnames <- list.files(".", pattern = "final_prolific_(.+).csv")
...
```{r}
df_db <- read_csv("final_db_allconds.csv", col_types = cols(flags = col_character()))
```

```{r}
df <- df_db %>%
  right_join(df_prolific %>% 
      select(prolific_pid, vis), by = "prolific_pid")  %>%
...
```{r}
# in case someone didn't complete all the trials
df_mask <- crossing(
  trial = 1:35,
  nregions = c(8,12)
)
...
## (Bonus calculation)
  ...
  df %>%
    filter(vis == "dotplot") %>%
    group_by(vis, prolific_pid) %>%
    summarise(end_pay = last(cumulative_pay)) %T>% print() %>%
    filter(end_pay >0 ) %>%
    group_by(vis) %>%
    mutate(bonus = (end_pay)/(max(end_pay)) * 1.5) %>%
    mutate(bonus = format(bonus, digits = 2)) %>%
    ungroup() %>%
