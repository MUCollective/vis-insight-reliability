...
data.sim <- crossing(
  ...
  mutate(
    ...
    true_mu = rnorm(N, a_bar, sigma) + mu_product + mu_region,
    sd = sigma / sqrt(Ni),
...
```{r, plot-raw, fig.width = 12, fig.height = 6}
for(i in 1:ntrials) {
    p <- data.sim %>%
        filter( trial == i) %>%
        unnest( profit ) %>%
    ...
    ggsave(filename = paste0('../plots/raw-data/profits-', i, '.png'), plot = p, device = "png")
}
...
```{r, plot-mean-bar, fig.width = 7, fig.height = 3}
data.sim %>%
...
```{r, plot-mean-ensemble, fig.width = 7, fig.height = 3}
...
```{r, plot-mean-ci, fig.width = 7, fig.height = 3}
data.sim.boot %>%
  unnest( boot.mean ) %>%
