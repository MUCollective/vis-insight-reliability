...
df.bh.mean <- df.bh %>%
  group_by(nregions) %>%
  summarise(tp = sum(tp), tn = sum(tn), fp = sum(fp), fn = sum(fn), positives = sum(is_profit), .groups = "drop") %>%
  pivot_longer(c(tp:fn), names_to = ".category", values_to = ".value") %>%
  rename( ntrials = nregions ) %>%
  mutate( 
    .category = toupper(.category),
    .value = .value / (ntrials * 35) 
  )
...
actual.statistics %>%
  ...
  geom_vline(data = df.bh.mean, aes(xintercept = .value), linetype = "dotted", color = "blue", size = 1) +
...
p1.fdr <- draws.fit %>%
  ...
  geom_text(data = data.fp_rate, aes(x = uncorrected_fp, y = factor(ntrials)), label = "|", size = 8) +
...
cowplot::plot_grid(p1.fdr, p2.fdr, rel_widths = c(6.5, 3.5))
```
...
draws.fit %>%
  ...
  ggplot(aes(x = fp_rate, y = factor(ntrials))) +
  geom_halfeyeh(.width = c(.95, .8, .5)) +
  geom_text(data = data.fp_rate, aes(x = uncorrected_fp, y = factor(ntrials), color = factor(ntrials)), label = "|", size = 8) +
  geom_vline(data = data.frame(xint = 0.05, .category = "FP"), aes(xintercept = xint), color = "black", alpha = 0.5) +
  labs(x = "False Discovery Rate", y = "Number of graphs shown") +
...
```{r, 08-probs-vis-learning}
draws.fit %>%
  group_by(condition, ntrials, .draw, trial_id, .category) %>%
  summarise(.value = mean(.value), .groups = "drop") %>%
  ggplot() +
  stat_lineribbon(aes(x = trial_id, y = .value), alpha = 0.05) +
  scale_fill_brewer() + 
  theme_minimal() +
  ylim(c(0, 0.65)) +
  theme(
...
draws.ci.sample <- sample_n(draws.ci, 1000)

p2.ci <- draws.ci.sample %>%
  ggplot() +
  geom_point(aes(x = trial_id, y = .value), alpha = 0.4, color = "#5B75BD") + #"#6ECCB0"
  theme_minimal() +
  ylim(c(0, 0.32)) +
  theme(
    axis.text = element_text(size = 12),
...
p3.ci <- draws.ci.sample %>%
  ggplot() +
  geom_dotplot(aes(x = .value), binwidth = 0.0025, stackratio = 1.4, fill = "#3182bd", color = NA) + #"#7BE3C4"
  xlim(c(0, 0.32)) +
  theme_minimal() +
  coord_flip() +
  theme(
...
p1 <- draws.fit %>%
  ...
  geom_text(data = df.f1, aes(x = fscore, y = factor(ntrials), color = factor(ntrials)), label = "|", size = 8) + 
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14)
...
p1.fscore <- draws.fit %>%
  ...
  geom_vline(data = df.f1, aes(xintercept = mean(fscore)), color = "#AB2C09") + 
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14)
...
cowplot::plot_grid(p1.mcc, p2.mcc)
```
...
p1 <- draws.fit %>%
  ...
  geom_vline(data = df.payout, aes(xintercept = mean(payout)), color = "#AB2C09") + 
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14)
