...
df <-  expand_grid(nregions = c(8, 12),
    iter = 1:n_iter) %>%
    split(1:nrow(.)) %>%
    map_dfr(~ bind_cols(.,
...
sim_df <- readRDS("sim_n5000_alpha_pnull_delta.rds")
```
...
df_stimuli %>%
  ...
  group_by(nregions, method, trial) %>%
  summarize( tp = sum(!true & reject),
             fp = sum(true & reject),
             tn = sum(true & !reject),
             fn = sum(!true & !reject), 
...
(p3 <- sim_df %>% 
  ...
  facet_grid(method ~ delta) + 
  labs(title="With simulated data, multiple iterations")
)
```
...
stimuli_alpha_df %>% 
  ...
  geom_vline(aes(xintercept = 0.05)) +
  facet_grid(method ~ delta) +
  labs(title = "With experimental stimuli, equiv to one iteration, no uncertainty")
)
