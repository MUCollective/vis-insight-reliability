...
```{r}
expand_grid(nregions = c(8, 12), iter = 1:2, nesting(tibble(mu = 0.9, sigma = 1))) %>%
  uncount(nregions, .remove = FALSE, .id = "panel") %>%
  group_by(iter, nregions) %>%
  mutate(
    mu = mu * rbinom(nregions, 1, 0.2), #simulating over different values of this
    p_raw = map2_dbl(mu, sigma, ~rp(.x, .y)), 
    p_bh = p.adjust(p_raw, method = "BH"),
    p_bonf = p.adjust(p_raw, method = "bonferroni"),
  ) %>%
...
Benjamini-Hochberg is the optimal strategy among all the three strategies around P(null) = 0.5 at $\alpha = 0.05$
...
```{r}
sim_df %>% 
  filter(p_null> 0.4 & p_null < 0.6 & delta == 0.6) %>%
  ggplot(aes(x = alpha, y = E_pay, color = method)) +
  geom_line() + 
  facet_grid(nregions ~ p_null) + 
  labs(title = "Which (multiple comparisons) correction method gives higher payout")
```

Checking the incentives: we plot the $\alpha$ value where payout is the highest, and see how close it is to 0.05 under different strategies.
