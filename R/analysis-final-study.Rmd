...
data.fp_rate <- select(data.sample, -c(p_h1, mean, sd, effect_size, population) ) %>%
  ...
  rename(ntrials = nregions) %>%
  group_by(ntrials) %>%
  summarise(fdr = mean(uncorrected_fp) / mean(selected), .groups = "drop")

data.bh.fp_rate <- pivot_wider(df.bh.mean, names_from = .category, values_from = .value) %>%
  group_by(ntrials) %>%
  summarise(fdr = FP / (TP + FP), .groups = "drop")
```
...
p1.fdr <- draws.fit %>%
  ...
  geom_text(data = data.bh.fp_rate, aes(x = fdr, y = factor(ntrials)), label = "|", size = 8) +
...
draws.ci.sample <- draws.ci %>%
  group_by(.draw) %>%
  summarise(trial_id = list(trial_id), .value = list(.value)) %>%
  sample_n(500) %>%
