...
### Expected payoff ~ P(null)

```{r}
tibble(p_null = 0) %>%
  ggplot(aes(x = p_null)) + 
  stat_function(fun = ~ E_payoff_raw(0.05, n_obs, .x, 1, 1, n_obs)) + 
  xlim(0, 1)
```
...
- [ ] Run code backwards to see which $\alpha$ level the -19 incentives are for, with R function to solve for the inverse [link](https://stackoverflow.com/questions/10081479/solving-for-the-inverse-of-a-function-in-r)
    - [ ] Run B-H with $\alpha = 0.13$ and calculate expected payouts as sanity check
    - [ ] Plot penalty v.s. $\alpha$
- [ ] Check $F_p$ equation: it's different from simulation using $p$-values to reject the nulls, see dashed lines on the last figure in Rmd
    - If $F_p$ is not appropriate (check assumptions!), use simulation to find out what the effective $\alpha$ is 

## Deriving FP incentive

- The rational user will make a selection (reject the null) if the reward of selecting is greater than not selecting
- Other side of the decision: the rational user will not make a selection (not reject the null) if the reward of not selecting is greater than selecting. The inequality for "reward if not rejecting the null < reward if rejecting the null"

$$
\begin{align}
dE[n_{TN}] - b E[n_{FN}] &< cE[n_{TP}] - a E[n_{FP}] \tag*{setting c, d = 1 because we can}\\
E[n_{TN}] - b E[n_{FN}] &< E[n_{TP}] - a E[n_{FP}]\\

 &< nP(p \ge \alpha|strategy,null)P(null)
\end{align}
$$

where $a, b$ are penalties for making FP, FN mistakes

$$
\begin{align}
[1 - 2 F_p(\alpha)] [1 - P(null)] &= [b ( 1 - \alpha) - a \alpha] P(null)\\
 a &= \frac{b (1-\alpha) - [1 - 2 F_p(\alpha)] [1 - P(null)] }{\alpha P(null) }
\end{align}
$$
...
tibble(alpha = 0) %>%
  ggplot(aes(x = alpha)) + 
  stat_function(fun = ~ fp_penalty(P_null = 0.5, .x)) + 
...
Take the inverse of FP penalty function, setting penalty = 19, P(null) = 0.5
...
fp_penalty_inv <- inverse(function(x) fp_penalty(P_null = 0.5, x), 0, 1 )
