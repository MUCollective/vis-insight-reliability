...
data.sim <- crossing( 
  ...
  unnest( cols = c(mu) ) %>%
  mutate( 
    region_idx = rep(1:n_regions, n_trials * length(std.dev)),
...
for(i in 1:n_regions) {
  for(j in 1:n_trials) {
    for (k in 1:length(std.dev)) {
      p <- data.sample %>%  
          unnest( cols = c(profit, store_idx) ) %>%
          filter( trial == j & region_idx == i & sd == k ) %>%
          ggplot(aes(x = NA, y = profit)) + 
          geom_jitter( width = 0.2, alpha = 0.7) +
          geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
          ylim(c(-6, 6)) +
        ...
        ggsave(filename = paste0('../plots/raw-data/profits-t', j,'-sd', k, '-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png") 
    }
  }
}
```
...
data.interval <- data.sample %>%
  ...
  select( - store_idx ) %>%
  filter( trial == 1 & region_idx == 1 & sd == std.dev[[1]] )
  ...
  for(j in 1:n_trials) {
    for (k in 1:length(std.dev)) {
      data.interval.i <- data.interval %>%
        filter( trial == j & region_idx == i & sd == std.dev[[k]] )
      ...
      ggsave(filename = paste0('../plots/ci/profits-t', j,'-sd', k, '-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png") 
    }
  }
}
```
