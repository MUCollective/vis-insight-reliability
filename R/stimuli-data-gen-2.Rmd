...
for(i in 1:n_regions) {
  for(j in 1:(n_trials * length(effect_sizes) * length(means))) {
    p <- data.sample %>%  
      ...
      ggsave(filename = paste0('../plots/raw-data/profits-t', j,'-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png") 
  }
}
```
...
```{r, mean_ci_save, fig.width = 3, fig.height = 4, eval = FALSE}
data.interval <- data.sample %>%
  mean_qi(profit, .width = c(.95)) %>%
  select( - store_idx )
...
for(i in 1:n_regions) {
  for(j in 1:(n_trials * length(effect_sizes) * length(means))) {
    ...
    p <- data.sample %>%  
      ...
      filter( trial == j & region_idx == i ) %>%
      ggplot(aes(x = NA, y = profit)) + 
      geom_pointinterval(data = data.interval.i) +
      geom_point( data = data.interval.i, aes(x = NA, y = profit), size = 4) +
      geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
      ylim(c(-15, 15)) +
      theme( 
        axis.text.x = element_blank(),
    ...
    ggsave(filename = paste0('../plots/ci/profits-t', j, '-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
...
for(i in 1:n_regions) {
  for(j in 1:(n_trials * length(effect_sizes) * length(means))) {
    p <- data.sim.boot %>%
      filter( trial == j & region_idx == i ) %>%
      unnest( cols = c(profit, store_idx) ) %>%
      mean_qi(boot.mean, .width = c(.95)) %>%
      ggplot(aes(x = NA, y = boot.mean)) + 
      geom_pointinterval() +
      geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
      ylim(c(-5, 6)) +
      ylab( "mean point estimate and 95% confidence interval for profit" ) +
      theme( 
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.x = element_blank(),
    ...
    ggsave(filename = paste0('../plots/boot-ci/profits-t', j, '-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
...
for(i in 1:n_regions) {
  for(j in 1:(n_trials * length(effect_sizes) * length(means))) {
    ...
    ggsave(filename = paste0('../plots/halfeye/profits-t', j, '-p0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
