...
```{r}
RNG_data = function (n_regions, n, p_null) {
  sobol(n_trials * n_regions, dim = n + 1, scrambling = 3, seed = .seed) %>%
...
Next, we set up the parameters which are relevant for our experiment design. Our study is designed to be analogous to NHST at $\alpha = 0.05$, which is the convention. We present participants with graphs of 20 points. To support multiple comparisons, we create graphs for at 6 different levels of "# of possible comparisons"

```{r}
alpha = 0.05
sample_size = 20
n_trials = 1e3
n_region = c(1, 2, 4, 8, 12, 16)
.seed = 1
```

Using these parameters, we see if we are able to generate data where, as the number of comparisons ($x$) increases, the probability of a False Positive finding also increases as: $f(x) = 1 - (0.95)^x$. We can see from our results that this holds:
...
data.sample = crossing(n_regions = n_region, p_null = 1) %>%
  ...
  mutate(data = map2(data, mu, ~ qnorm(.x, .y))) %>%
  mutate( p = map_dbl(data, ~ t.test(.x, alternative = "greater")$p.value) ) %>% # calculates p-value
...
```{r}
data.sample %>%
  summarise(FPR = any(FP), .groups = "drop_last") %>%
  summarise(FPR = mean(FPR), .groups = "drop_last") %>%
...
```{r}
(data.sample.grid = crossing(n_regions = n_region, p_null = seq(0, 1, by = 0.1)))
```
...
```
Next, we process the data to be in the long format, which would make subsequent operations eaasier:

```{r}
data.sample.unif = data.sample.unif %>%
  pivot_longer(cols = starts_with("store_"), names_to = "store", names_prefix = "store_", values_to = "profit") %>%
  group_by(n_regions, p_null, trial, region, mu) %>%
  summarise(data = list(profit), .groups = "drop")
...
Recall that the column data currently stores the quantiles for the profit of 20 stores, based on a normal distribution. However, we want the actual profits. In the next step, we compute profits from the available quantile information using the `qnorm` function. We use mean and SD of 1.

```{r}
data.sample = data.sample.unif %>%
  group_by(n_regions, p_null, trial) %>%
  mutate(data = map2(data, mu, ~ qnorm(.x, .y)))

saveRDS(data.sample, "sim_data_sample_n1000.rds")

head(data.sample)
```

Now that we have actual data for all the 20 store (in each of varying number of regions and values of $p(null)$, over 1000 trials), we can perform a NHST using a one-sided t-test. This will give us an uncorrected p-value. We can then account for multiple comparisons by adjusting the p-values using various correction methods such as Benjamini-Hochberg and Bonferroni:

```{r}
data.sample = readRDS("sim_data_sample_n1000.rds") # delta = 1
```

Maybe the problem with payout is because the $\delta$ this time is larger???
Last time: $\delta = [0.3, 0.45, 0.6]$


ACHTUNG: secretly changing delta

```{r}
delta <- 0.5

data.sample <- data.sample %>%
  mutate(data = map2(data, mu, \(x, mu) mu + (x - mu) / delta)) 
...
Next, we compute the TP/TN/FP/FN values for each which then allows up to compute False Discovery Rates

```{r}
data.sample.stats2 = data.sample.stats %>%
...
```{r checking BH}
# average/expected? FDR across the 1000 trials
df_fdr_payout %>%
  group_by(n_regions, p_null, method) %>% 
  summarize(FDR = mean(FDR), .groups = "drop")  %>%
  filter(method == "BH" & FDR > 0.05)
```
...
Is OK if FWER is > 0.05 from the simulation because p-value is uniformly distributed? $P(null) = 0.9$ and 1 are more problematic because ....?
...
data.sample.stats %>%
  ...
  labs(title = "distribution for p-value under H0")
```
ACHTUNG: why high `n_regions` and high `p_null` leads to high FWER/FDR

```{r}
df_alpha_adj <- tibble(n_regions = n_region) %>%
  mutate(p_adj = 0.05/ n_region )

data.sample.stats %>%
  filter(method == "uncorrected", mu == 0) %>%
  filter(p <= 0.05, n_regions == 16) %>%
  ggplot(aes(x = p)) + 
  geom_histogram() + 
  facet_wrap( ~ p_null, labeller = label_both) +
  geom_vline(data= df_alpha_adj %>% filter(n_regions == 16), mapping = aes(xintercept = p_adj)) +
  labs(title = "faceted by P(null)")
```

```{r}
data.sample.stats %>%
  filter(method == "uncorrected") %>%
  group_by(n_regions, region) %>%
  slice(3)  %>%
  arrange(region)
```
...
data.sample.stats2 %>%
  ...
  ggplot() +
  geom_point(aes(x = p_null, y = FDR, color = method)) +
  geom_hline(yintercept = 0.05, color = "red") +
  facet_wrap( ~ n_regions)
```

### check if BH is still the best strategy

Looks like Bonferroni is the better one, 

```{r}
data.sample.stats2 %>%
  summarise_at(.vars = vars(TP, FP, TN, FN), sum) %>%
  mutate(payout = TP + TN - FN - (19*FP)) %>%
  summarise(E_payout = mean(payout), .groups = "drop") %>%
  ggplot() +
  geom_line(aes(x = p_null, y = E_payout, color = method)) +
  facet_wrap( ~ n_regions)
```
Closer look at the FP and payout patterns

```{r}
df_fdr_payout %>%
  filter(method != "uncorrected") %>%
  group_by(n_regions, p_null, method) %>%
  mutate(across(TP:FN, \(x) x / n_regions, .names = "{.col}R")) %>%
  summarize(across(FDR:FNR, ~ mean(.))) %>%
  pivot_longer(cols = FDR:FNR) %>%
  mutate(name = fct_relevel(name, c("TPR", "FPR", "TNR", "FNR"))) %>%
  ggplot() +
  geom_line(aes(p_null, value, color = method, group = interaction(name, method, n_region ))) +
  facet_grid(name ~ n_regions, scales = "free_y")
```
...
### Checks: data generation

We want to make sure that our generated data for the stimuli satisfies the following two criteria:
- the # FPs under an uncorrected strategy be monotonically increasing with # regions
- the generated dataset is not an outlier (we test this by comparing the FP statistics to a a dsitribution of 1000 other datasets generated using the same process).
...
```{r}
simulate_data = function(p_h1, regions) {
  mu = rbinom(regions, 1, p_h1)
  region_idx = seq(1:regions)
  data = map(as.list(mu), ~ rnorm(sample_size, mean = .))
  
  tibble(mu = mu, region_idx = region_idx, n_region = regions) %>%
    mutate(
      data = as.list(data),
      p = map_dbl(data, ~ t.test(.x, alternative = "greater")$p.value),
      reject = p < alpha, true = mu == 0
    ) %>%
    group_by(n_region) %>%
...
sim_df = d %>%
  split(1:nrow(.)) %>%
  future_map_dfr( ~ cbind(.x, simulate_data(.$p_h1, .$n_region), row.names = NULL),
                 .options = options, .progress = TRUE)

beep()
saveRDS(sim_df, "sim_n1000.rds")
```


```{r}
sim_df = readRDS("sim_n1000.rds")

sim_df %>%
  mutate(n_region = as.factor(n_region)) %>%
  group_by(iter, n_region) %>%
  summarise(FDR = mean(FDR), .groups = "drop") %>%
  ggplot() +
  geom_point(aes(x = FDR, y = n_region), position = position_jitter(height = 0.2), alpha = 0.1) +
  geom_point(data = data_sample.summary, aes(x = FDR, y = factor(n_regions)), color = "red")
```
...
select(data.sample, -c(p_h1, -effect_size, mean, sd) ) %>%
  mutate(n_region = as.numeric(n_region)) %>%
  mutate(
...
# saveRDS(data.sim, file = "../data/simulated_data-all_regions.rds")
```


```{r, remove-later, eval = FALSE}
# remove later
...
```{r}
# data.sample <- rbind(
#   mutate(data.sim.12reg, nregions = 12),
#   mutate(data.sim.8reg, nregions = 8)
# )
