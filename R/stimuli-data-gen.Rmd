...
set.seed(97674125) #7841, 753614, 97674125
sample_size = 20
n_trials = 10
n_stores = 20
n_region = c(12, 8)
...
```{r}
## calculates the number of regions one should select on average using Benjamini-Hochberg strategy vs the raw p-value
rbind(
  select(data.sim.12reg, -c(effect_size, p_h1)) %>%
    mutate(
      p_value = round(map_dbl(data, ~ t.test(.x)$p.value), 5),
      alpha = 0.05 #map(trial, ~ c(0.1, 0.05, 0.01))
    ) %>%
    unnest(cols = c(alpha)) %>%
    group_by(trial, alpha, sd) %>%
    arrange(alpha, trial, p_value) %>%
    mutate(
      rank = 1:n_region[[1]],
      critical_value = rank/n_region[[1]] * 0.05,
      bh_profit = ifelse(p_value < critical_value, TRUE, FALSE),
      raw_profit = ifelse(p_value < alpha, TRUE, FALSE)
    ) %>%
    summarise(bh_select = sum(bh_profit), raw_select = sum(raw_profit)) %>%
    ungroup() %>%
    mean_qi(bh_select, raw_select),
  select(data.sim.8reg, -c(effect_size, p_h1) ) %>%
    mutate(
      p_value = round(map_dbl(data, ~ t.test(.x)$p.value), 5),
      alpha = 0.05 #map(trial, ~ c(0.1, 0.05, 0.01))
    ) %>%
    ...
    mutate(
      rank = 1:n_region[[2]],
      critical_value = rank/n_region[[2]] * 0.05,
      bh_profit = ifelse(p_value < critical_value, TRUE, FALSE),
      raw_profit = ifelse(p_value < alpha, TRUE, FALSE)
    ) %>%
    summarise(bh_select = sum(bh_profit), raw_select = sum(raw_profit)) %>%
...
data.sim.12reg %>%  
  ...
  geom_jitter( width = 0.2, alpha = 0.7, alpha = 0.5) +
...
```{r, plot-mean-density}
data.sim.boot.12reg %>%
  filter(trial == 13 & region_idx == 1) %>%
  unnest( cols = c(boot.mean) ) %>%
  ggplot( aes(x = 0, y = boot.mean) )  +
  stat_dots(position = "dodge", quantiles = 20, binwidth = 0.5, fill = "#4682B4", color = "#ffffff") +
  geom_hline( yintercept = 0, alpha = 0.5, color = "red" ) +
