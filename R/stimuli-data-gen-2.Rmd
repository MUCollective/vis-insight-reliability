...
for(i in 1:n_regions) {
  p <- data.sample %>%
    ...
    rename(sample = profit, population = data) %>%
    mutate( 
      pop_mean = limits[2] - 1 + runif(n()),
      mean = limits[2] -1 + runif(n()),
      se = runif(n()) * 0.1 + 0.1,
      .lower = qTF(0.025, mean, se, nu = nu),
      .upper = qTF(0.975, mean, se, nu = nu)
    ) %>%
  ...
  ggsave(filename = paste0('../plots/attention/profit-','ci' ,'-', i, '.png'), height = 8, width = 5, units = "in", dpi = 72, type = "cairo-png", plot = p, device = "png")
}
```
...
for(i in 1:n_regions) {
  p <- 
    tibble(y = rep(limits[2] - 1.5, sample_size)) %>%
    ggplot() + 
    geom_point(aes(x = NA, y = y), position = "jitter") + 
    geom_hline( yintercept = 0, size = 1, color = "red", alpha = 0.5 ) +
    scale_y_continuous( limits = limits, breaks = seq(limits[[1]], limits[[2]], by = 2) )  +
    theme(legend.position = "none") +
    ylab( "profit" )

  ggsave(filename = paste0('../plots/attention/profit-','raw-data' ,'-', i, '.png'), height = 8, width = 5, units = "in", dpi = 72, type = "cairo-png", plot = p, device = "png")
}
```
```
