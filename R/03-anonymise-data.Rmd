...
prolific_pids = c(
  filter(
    ...
    Completion.code == "CDK4LRCQ")$Participant.id)
```

Note: there are some issues with two participants, who we exclude. For anonymity reasons, we do not disclose their Prolific IDs. These participants appear to have taken the survey twice, presumably because they failed the attention check in their first attempt. We will filter them out
...
```{r load_data}
dat = read.csv("../data/final/02-data-qualtrics.csv") |> 
  filter(row_number() > 2) |> 
  rename(
  ...
  select(duration, resolution, Q96, X1_8regions_train_q1:X11_20regions_trial_q20, vis, trial_index_map, block_order, cummu_pay, user_id, nregions) 
...
```{r}
dat |> 
  mutate(avg_pay = strtoi(cummu_pay)/30) |> 
  select(user_id, Q96, vis, cummu_pay, avg_pay) |> 
  write.csv("../data/final/03-qualitative-responses.csv", row.names = FALSE)
```
...
df.responses.long |> 
  ...
  mutate(
    ...
    bonus = round(9*payoff / max(payoff))/2
  ) |> 
  arrange(payoff) |> 
...
```{r}
# this creates a new DF where participants IDs are anonymised and the data is in the long format
