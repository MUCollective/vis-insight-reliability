...
df.bh <- select(data.sample, -c(p_h1, -effect_size, mean, sd, population) ) %>%
...
df.bh %>%
  group_by(trial, nregions) %>%
  summarise(fp_bh = sum(fp), .groups = "drop") %>%
  ggplot(aes(x = factor(trial), y = fp_bh, fill = factor(nregions))) +
  geom_col()
```
...
data.fp_rate <- select(data.sample, -c(p_h1, mean, sd, effect_size, population) ) %>%
  mutate(
    p_value = round(map_dbl(data, ~ t.test(.x, alternative = "greater")$p.value), 5),
