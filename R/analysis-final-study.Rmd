...
ame_draws %>% ggplot(aes(x = .value, y = condition)) + 
  stat_ccdfinterval(fill = "#A5C4D4", color = "#393D3F") + 
  geom_point(data = participant_means, color = "#F5B700")+
   geom_vline(data = filter(df.strategy_by_trial, method=="bh"), aes(xintercept = .value), color = col_bh) +
  geom_vline(data = filter(df.strategy_by_trial, method=="uncorrected"), aes(xintercept = .value), color = col_uncorrected) +
  geom_vline(data = data.frame(xint = 0.05, .category = "FDR"), aes(xintercept = xint)) +
  facet_grid(factor(ntrials) ~ .category, scales = "free_x")   + # space = "free_x"
  theme_classic()

ggsave("analysis-final-study_files/figure-latex/viscond-nregions-tp-fdr.pdf", width = 24, height = 6)
