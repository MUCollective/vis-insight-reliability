...
```{r, scatterplot_save, fig.width = 3, fig.height = 4, warning = FALSE}
for(i in 1:n_regions) {
  for(j in 1:n_trials) {
    p <- data.sample %>%  
      unnest( cols = c(profit, store_idx) ) %>%
      filter( trial == j & region_idx == i & p_h1 == 0.5 ) %>%
      ggplot(aes(x = NA, y = profit)) + 
      geom_jitter( width = 0.2, alpha = 0.7) +
      geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
      ylim(c(-6, 6)) +
    ...
    ggsave(filename = paste0('../plots/raw-data/profits-t', j,'-p_0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
...
for(i in 1:n_regions) {
  for(j in 1:n_trials) {
    ...
    p <- data.sample %>%  
      unnest( cols = c(profit, store_idx) ) %>%
      filter( p_h1 == 0.5 & p_h1 == j & region_idx == i ) %>%
      ggplot(aes(x = NA, y = profit)) + 
    ...
    ggsave(filename = paste0('../plots/ci/profits-t', j,'-p_0.5-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
