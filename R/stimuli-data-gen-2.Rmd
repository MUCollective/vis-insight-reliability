...
data.sim <- crossing( 
  ...
  unnest( cols = c(mu) ) %>%
  mutate( 
    region_idx = rep(1:10, (length(p_h1_levels) * 10)),
    data = map2(mu, sd, ~ rnorm(n_stores, .x, .y))
  )

p_h1_levels = p_h1_levels / 10
```
...
### Simulations
Let's run some simulations to calculate if the number of panels will have an effect on FDR detection
...
data.sample %>% 
  ...
  group_by(trial) %>%
  sample_n( n_plots ) %>%
  mutate( p_val = map_dbl( profit, ~t.test(.x)$p.value ) ) %>%
  arrange( p_val, .by_group = TRUE ) %>%
  mutate( 
    rank = seq(1:n_plots),
    critical_bh = (rank/n_plots) * fdr,
    reject = ifelse( p_val <= critical_bh, TRUE, FALSE)
  )
```
...
```{r, scatterplot, fig.width = 3, fig.height = 4, warning = FALSE}
data.interval <- data.sample %>%
  mean_qi(profit, .width = c(.95)) %>%
  select( - store_idx )

for(i in 1:n_regions) {
  for(j in 1:length(p_h1_levels)) {
    data.interval.i <- data.interval %>%
      filter( trial == 1  & p_h1 == p_h1_levels[j] & region_idx == i )
    ...
    ggsave(filename = paste0('../plots/ci/profits-p_', p_h1_levels[j], '-', i, '.png'), height = 6, width = 4, units = "in", dpi = 150, plot = p, device = "png")
  }
}
```
...
data.interval <- data.sample %>%
  mean_qi(profit, .width = c(.95)) %>%
  select( - store_idx ) %>%
  filter( trial == 1 & p_h1 == 0.5 & region == "YDGA" )
