...
data.sample %>%
  mutate(
    estimate = round(map_dbl(profit, ~t.test(.x, conf.level = 0.9)$p.value), digits = 5),
...
p2 <- data.example %>%
  ggplot() + 
  geom_errorbar( aes(x = NA, ymin = .lower, ymax = .upper), width = 0, size = 3 ) +
  geom_point( aes(x = NA, y = boot.mean), size = 8 ) +
  geom_hline( yintercept = 0, color = "red", alpha = 0.5 ) +
  ylim(limits) +
  ylab( "mean profit and 95% confidence interval" )


pdf(file = "../figures/ci_calc_plot.pdf", useDingbats = FALSE, width = 8, height = 10)
