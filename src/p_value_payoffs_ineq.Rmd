...
The alpha level implied by our incentives is around 0.08 when P(null) is around 0.5; there is variation since we sampled P(null) from a binomial.
...
fp_penalty_inv <- function(incentive, P_null, l = 0, u = 1) {
  uniroot(function(alpha) fp_penalty(P_null, alpha) - incentive, lower = l, upper = u)$root
}

fp_penalty_inv(19, P_null = 0.5)

seq(from = 0.05, to = 0.95, length.out = 10) %>%
  map_dbl(~ fp_penalty_inv(19, .x))

tibble(pnull = seq(from = 0.05, to = 0.95, length.out = 100) ) %>%
...
## Simulating payoff with wrong alphas

```{r}
alpha = .05
n_panels = 10 # number of regions
n_obs = 20 # number of observations within each region
